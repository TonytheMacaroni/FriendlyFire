...
@HookInfo(
    name = "entity-mount",
    description = "Controls mounting entities.",
    namespace = "friendlyfire"
)
public class EntityMountEventHook extends Hook<EventPriorityConfig> implements EventHook {

    public EntityMountEventHook(EventPriorityConfig config) {
        super(config);
    }
    ...
    public Collection<Listener> collectListeners() {
        return List.of(
            EventListener.of(EntityMountEvent.class, this)
                .priority(config.eventPriority)
                .actor(EntityMountEvent::getEntity)
                .target(EntityMountEvent::getMount, Relation.ALLIED)
                .build()
        );
    }
